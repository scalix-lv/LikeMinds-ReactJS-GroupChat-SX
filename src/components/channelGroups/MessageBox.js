...
function MessageBox({ username, messageString, time, userId }) {
  return (
    <Box className="flex">
      <StringBox
        username={username}
...
function StringBox({ username, messageString, time, userId }) {
  return (
    <Box
      className="flex justify-between py-4 px-5 w-[270px] rounded-[10px]"
      sx={{
    ...
    <Box className="flex items-center">
      ...
      <Menu open={open} anchorEl={anchor}>
        {options.map((option, optionIndex) => {
          return <MenuItem key={option.title}>{option.title}</MenuItem>;
        })}
      </Menu>
    </Box>
  );
